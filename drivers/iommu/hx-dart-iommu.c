...
static bool hx_dart_iommu_capable(struct device *dev, enum iommu_cap cap)
{
    switch (cap) {
...
static struct iommu_domain *hx_dart_iommu_domain_alloc_paging(struct device *dev)
{
...
static int hx_dart_iommu_attach_device(struct iommu_domain *domain,
...
static int hx_dart_iommu_map_pages(struct iommu_domain *domain,
...
static size_t hx_dart_iommu_unmap_pages(struct iommu_domain *domain,
...
static int hx_dart_iommu_iotlb_sync_map(struct iommu_domain *domain,
...
static struct iommu_device *hx_dart_iommu_probe_device(struct device *dev)
{
    struct hx_dart_iommu_devdata *idd = dev_iommu_priv_get(dev);
...
static void hx_dart_iommu_release_device(struct device *dev)
{
    struct hx_dart_iommu_devdata *idd = dev_iommu_priv_get(dev);
...
static struct iommu_group *hx_dart_iommu_device_group(struct device *dev)
{
#ifdef CONFIG_PCI
    if(dev_is_pci(dev))
...
static int hx_dart_iommu_of_xlate(struct device *dev, const struct of_phandle_args *args)
{
...
static const struct iommu_ops hx_dart_iommu_ops = {
    ...
    .domain_alloc_paging = hx_dart_iommu_domain_alloc_paging,
    .probe_device = hx_dart_iommu_probe_device,
    .release_device = hx_dart_iommu_release_device,
    .device_group = hx_dart_iommu_device_group,
    .owner = THIS_MODULE,
    .default_domain_ops = &(const struct iommu_domain_ops) {
        .attach_dev = hx_dart_iommu_attach_device,
        .map_pages = hx_dart_iommu_map_pages,
        .unmap_pages = hx_dart_iommu_unmap_pages,
        ...
        .free = hx_dart_iommu_domain_free,
    },
};
...
static int hx_dart_iommu_probe(struct platform_device *pdev)
{
    ...
    ret = iommu_device_register(&im->iommu, &hx_dart_iommu_ops, dev);
    if(ret)
        return ret;
...
static void hx_dart_iommu_remove(struct platform_device *pdev)
{
}
